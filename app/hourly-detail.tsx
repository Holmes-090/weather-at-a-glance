...
// Custom SVG Chart Component for Precipitation (Combined Bar + Line Chart)
function PrecipitationChart({ data, width, height }: {
  ...
  return (
    <Svg width={width} height={height}>
      ...
      {/* Line data points - show every 3rd point to reduce clutter */}
      ...
      {/* X-axis labels - show every 3rd hour to reduce clutter */}
      {data.map((point, index) => {
        if (index % 3 !== 0) return null; // Show every 3rd label
        const x = padding.left + (index * barSpacing) + barSpacing / 2;
        return (
          <SvgText
            key={index}
...
export default function HourlyDetailScreen() {
  ...
  const precipitationData = useMemo(() => {
    ...
    return hourlyData.map((hour, index) => ({
      x: index,
      precipMm: hour.precipitationMm || 0,
      precipChance: hour.precipitationProb || 0,
      label: hour.label,
    }));
  }, [hourlyData, mode]);
